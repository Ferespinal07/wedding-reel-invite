...
const copy = {
  ...
  en: {
    ...
    outfitTitle: "OUTFITS",
    outfitScript: "for the wedding",
    suggestedColors: "Suggested colors:",
    elegantLooks: "Elegant, fresh, timeless looks to celebrate this very special day with us.",
    inspire: "Be inspired",
    inspireText: "by the color palette and choose a look that makes you feel incredible.",
  },
};
        ...
        <div className="mt-12 grid w-full gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
          ...
          >
            <div
              ...
              aria-label={text.infoAlt}
            >
              <div className="absolute inset-x-0 top-0 h-1 bg-[linear-gradient(90deg,#9aa28a,#77784d,#d2a094,#ccb29b,#e8dccb,#ac7d55)]" />
              <Sparkles className="absolute right-6 top-7 h-5 w-5 text-rose/50" strokeWidth={1.1} />
              <p className="font-serif text-5xl font-light uppercase leading-none tracking-[0.16em] text-[#6f7152]">
                {text.outfitTitle}
              </p>
              <p className="mt-1 font-serif text-4xl font-light italic leading-none text-[#c99e9a]">
                {text.outfitScript}
              </p>

              <div className="mx-auto mt-8 flex items-center justify-center gap-4 text-sage-deep/45">
                <span className="h-px w-20 bg-current" />
                <Heart className="h-5 w-5" strokeWidth={1.1} />
                <span className="h-px w-20 bg-current" />
              </div>

              <div className="mt-9 text-left">
                <p className="font-serif text-2xl text-[#74765a]">{text.suggestedColors}</p>
                <p className="mt-3 text-sm leading-7 tracking-wide text-foreground/65">
                  {text.paletteText}
                </p>
              </div>
              ...
              <div className="mt-8 grid grid-cols-6 gap-3">
                {swatches.map((color) => (
                  <span
                    key={color}
                    className="aspect-square rounded-full border border-white/80 shadow-[0_10px_25px_-18px_oklch(0.28_0.02_140/0.75)]"
                    style={{ backgroundColor: color }}
                  />
                ))}
              </div>
